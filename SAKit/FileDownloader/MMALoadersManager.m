                                    ...
                                    completed:(void (^)(NSURLResponse *response, NSString *filePath, NSError *error, MSTFileDownloadToken *tokenTask))completedBlock;
{
    ...
    __weak __block MSTFileDownloadToken *downloadToken = [self.allDownloadToken objectForKey:objectForKey];
    @weakify(self);
    if (!downloadToken || !downloadToken.downloadOperationCancelToken) {
        downloadToken = [[MSTFileDownloader sharedDownloader] downloadFileWithURL:url ownFileName:ownFileName taskIdentifier:taskIdentifier progress:^(NSProgress *downloadProgress, NSString *taskIdentifier) {
            ...
            if (completedBlock) {
                completedBlock(response, filePath, error, downloadToken);
            }
...
{
    [[MSTFileDownloader sharedDownloader] cancelAllDownloads];
}
