...
- (void)start
{
    @synchronized (self) {
        ...
        self.dataTask = [self.outerManager downloadTaskWithRequest:self.request progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            NSString *filePath;
            if (self.ownFileName) {
                filePath = [dir stringByAppendingPathComponent:[NSString stringWithFormat:@"%@%@",self.ownFileName, response.suggestedFilename.pathExtension]];

            }else {
                NSString *diskCachePath = SADiskCacheFileNameForKey(response.URL.absoluteString);
                self.ownFileName = diskCachePath;
                filePath = [dir stringByAppendingPathComponent:diskCachePath];
            }
            ...
            if (!error) {
                /// zip才需要解压
                if ([response.suggestedFilename.pathExtension isEqualToString:@"zip"]) {
                    ...
                    [self callCompletionBlocksWithFilePath:[filePath.absoluteString stringByRemovingPercentEncoding] response:response error:error];
                    [self done];
                }
